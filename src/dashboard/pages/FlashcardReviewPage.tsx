...
const FlashcardReviewPage = () => {
  ...
  const [reviewerPreferenceKey, setReviewerPreferenceKey] = useState<string>('')
  ...
  useEffect(() => {
    ...
    const loadReviewer = async () => {
      ...
      const preferenceKey = record?.reviewerPreference ?? ''

      setReviewerOutput(record?.aiReviewerOutput?.trim() ?? '')
      setActiveStage(activeStage)
      setReviewerPreferenceKey(preferenceKey)
      setReviewerPreference(preferenceLabels[preferenceKey] ?? 'Not set')
  ...
  if (reviewerPreferenceKey === 'audiobook') {
    return <Navigate to={ROUTE_PATHS.dashboard.reviewAudiobook} replace state={{ redirectNotice }} />
  }

  if (reviewerPreferenceKey === 'cheatsheet-pdf' || reviewerPreferenceKey === 'cheatsheet-image') {
    return <Navigate to={ROUTE_PATHS.dashboard.reviewCheatsheet} replace state={{ redirectNotice }} />
  }
